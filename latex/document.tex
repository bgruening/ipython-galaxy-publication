...
\section{Summary:}
The Galaxy Tool Shed allows for the easy installation of tools while also managing dependencies and updates of the tools. This makes the installation of new tools easy for an administrator. However, if tools are not available in the Galaxy Tool Shed a Galaxy administrator often can not help a user directly and the user has to resort to leaving the Galaxy environment and download their data for further analysis. The integration of IPython in Galaxy allows for an analysis of data within the Galaxy framework with the full versatility of Python and its scientific libraries. This bridges the gap between users with programming experience and Galaxy by adding the flexibility for the user to quickly program a solution for his problem staying within the Galaxy framework. IPython is executed in a Docker container which is invoked and terminated automatically. This design provides a secure framework for the user and the Galaxy system.
