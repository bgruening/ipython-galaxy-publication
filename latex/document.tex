...
% RFY: (on "the task...friendly workflow") This should go first in the
% paragraph. The idea is that the Galaxy toolshed has limited plotting routines
...
% RFY: There is a mismatch between "the user" and "their". In general, try to avoid possessive pronouns
...
% RFY: This seems a bit casual...needed? 
% ESR: probably should change to a more technical version of saying "hey, it's
% faster when you don't try and install 5 pieces of software yourself and then
% transfer gigs of data across the internet" 
...
\texttt{API\_KEY} without displaying their API key, which is invaluable if the user intends to publishs their notebook as
...
% RFY: "sanboxes", is this jargon? If not, reference. I think just saying "isolates"
isolating IE users from the rest of the Galaxy server, protecting the system and keeping sensitive data secure.
The user is additionally restricted by limits which administrators have placed on Docker containers,
